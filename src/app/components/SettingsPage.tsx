      ...
      <div className="flex gap-1 p-1 rounded-full bg-gray-100/80 backdrop-blur-sm border border-gray-200/50 w-fit">
        {tabs.map((tab) => (
          ...
          >
            ...
            <tab.icon
              className={`relative z-10 w-3.5 h-3.5 transition-colors ${activeTab === tab.id ? "text-emerald-500" : "text-gray-400"}`}
            />
            <span
              className={`relative z-10 ${activeTab === tab.id ? "text-emerald-700" : "text-gray-500"}`}
            >
              {tab.label}
            </span>
          </button>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2.5 rounded-xl bg-gray-50 border border-gray-200 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 transition-all"
            ...
            <GlassCard className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div
                    className="px-3 py-2.5 rounded-xl bg-emerald-50/50 border border-emerald-200 text-emerald-700"
                    style={{ fontSize: "0.85rem" }}
                  >
                    {invoicePrefix}-001 · {CURRENCIES[defaultCurrency]?.symbol}{" "}
                    · {defaultTax}% tax · Net {defaultPaymentTerms}
                  </div>
                </div>
              </div>
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-3 py-2.5 rounded-xl bg-gray-50 border border-gray-200 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 resize-none transition-all"
                  style={{ fontSize: "0.85rem" }}
                />
              </div>
            </GlassCard>
            <button
              onClick={() => toast.success("Invoice defaults saved!")}
              className="flex items-center gap-2 px-6 py-3 bg-emerald-500 text-white rounded-xl hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-500/20"
              style={{ fontWeight: 500 }}
            >
              ...
              {[
                {
                  ...
                  desc: "Automatically remind clients before due date",
                  value: autoReminders,
                  setter: setAutoReminders,
                },
                {
                  label: "Email Notifications",
                  desc: "Get notified when invoices are viewed or paid",
                  value: emailNotifications,
                  setter: setEmailNotifications,
                },
                {
                  label: "Payment Confirmations",
                  desc: "Receive email when a payment is processed",
                  value: paymentNotifications,
                  setter: setPaymentNotifications,
                },
                {
                  label: "Overdue Alerts",
                  desc: "Get alerted when invoices become overdue",
                  value: overdueAlerts,
                  setter: setOverdueAlerts,
                },
              ].map((item, i) => (
                <div key={i} className="flex items-center justify-between py-1">
                  ...
                  >
                    <div
                      className={`w-5 h-5 rounded-full bg-white shadow transition-transform ${item.value ? "translate-x-5" : ""}`}
                    />
                  </button>
                </div>
              ))}
                  ...
                  <div className="flex items-center gap-1 p-1 rounded-full bg-gray-100/60 border border-gray-200/50 w-fit">
                    {[1, 3, 5, 7, 14].map((d) => (
                      ...
                      >
                        ...
                        <span
                          className={`relative z-10 ${reminderDays === d ? "text-white" : "text-gray-600"}`}
                        >
                          {d} day{d > 1 ? "s" : ""}
                        </span>
                      </button>
